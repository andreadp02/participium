...
describe("authService", () => {
  ...
  describe("getMunicipalityUsers", () => {
    it("calls authRepository.getUsersByRole with MUNICIPALITY and returns users", async () => {
      const users = [makeUser(), makeUser({ id: 2 })];
      (authRepository.getUsersByRole as jest.Mock).mockResolvedValue(users);

      const res = await authService.getMunicipalityUsers();

      expect(authRepository.getUsersByRole).toHaveBeenCalledWith(
        roleType.MUNICIPALITY
      );
      expect(res).toBe(users);
    });

    it("propagates errors from authRepository.getUsersByRole", async () => {
      (authRepository.getUsersByRole as jest.Mock).mockRejectedValue(
        new Error("repo fail")
      );
